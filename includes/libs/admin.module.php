...
class admin {
	...
	function index() {
		...
		section_content('
			<IFRAME SRC="http://www.jcow.net/news.php?s='.urlencode($s).'&e='.urlencode($client['email']).'fn='.urlencode($client['fullname']).'&m='.$member['num'].'&br='.$_SESSION['br'].'" TITLE="Jcow News" WIDTH="580" HEIGHT="180" scrolling="no">
<a href="http://www.jcow.net">Visit Jcow for News&Updates</a><br />
</IFRAME>
		');
		section_close('Jcow News');
		$res = sql_query("select count(*) as num from ".tb()."accounts " );
		...
		section_content('
		<ul>
		<li>Your Jcow Version: <strong>'.jversion().'</strong> [<a href="http://www.jcow.net/download/" target="_blank"> <strong>Upgrade</strong> to Professional</a>]</li>
		<li>You have <a href="'.url('admin/users').'"><strong>'.$members.'</strong> Members</a>. (<strong>'.$pending_members.'</strong> pending)</li>
		<li>
		...
		'<br />'.url('admin/customfields',t('Profile Questions')).'</div>
	...
	if (get_gvar('private_network')) {
		...
		<option value="0" '.$story_access_all.' >Public - everyone can visite your site</option>
		<option value="1" '.$story_access_member.' >Private - only members can visite your site</option>
		</select>
		</p>
		');
		if (get_gvar('only_invited')) {
			$only_invited = 'selected';
		}
		else {
			$only_invited_no = 'selected';
		}
		...
		if (!$pending_post_limit = get_gvar('pending_post_limit'))
			$pending_post_limit = 0;
		section_content('
		
		<p>
		'.label('Account verification').'
		<select name="acc_verify">
		<option value="0" '.$acc_verify_0.' >Auto verified</option>
		<option value="1" '.$acc_verify_1.' >Verified by sending confirm code via Email</option>
		<option value="2" '.$acc_verify_2.' >Verified manually by Admin</option>
		</select><br />
		Pending members can submit up to <input type="text" size="3" name="pending_post_limit" value="'.$pending_post_limit.'" /> posts before being verifed. (recommended: 5)
		</p>
		</fieldset>');
		...
		<p>
		<input type="checkbox" value="1" name="enable_recaptcha_login" '.$enable_recaptcha_login_checked.'  />Enable reCaptcha in Login form.(Recommend to enable)
		</p>
		</fieldset>');
	...
	function users($filter = '') {
		...
		while ($member = sql_fetch_array($res)) {
			...
			section_content('<li><span>'.url('admin/useredit/'.$member['id'],$member['username'].' '.$member['lastname']). $status.'</li>');
		}
		section_content('</ul>');
	...
	function useredit($uid = 0) {
		...
		<p>
		<label>User IP</label>
		<strong>'.$user['ipaddress'].'</strong> (<a href="http://www.google.ca/search?q='.$user['ipaddress'].'" target="_blank">Whois</a>)<form action="'.url('admin/blacklist').'" method="post">
		...
		section_content('
					...
					<p>
					'.label(t('Status')).'
					<input type="radio" name="disabled" value=0 '.admin_check_status($user,0).' /> Active 
					<input type="radio" name="disabled" value=1 '.admin_check_status($user,1).' /> Pending 
					<input type="radio" name="disabled" value=2 '.admin_check_status($user,2).' /> Suspended<br />
	...
	function ad_blocks() {
		...
		section_content(' - Add your advertisements here. (i.e, google adsense)<br />
		 - All the content supports HTML!<br />
		 - We have optimized the AD spot for better clicking rate.<br />
		 - Ads will be hidden in "Special" pages like "Edit Profile","Sign Up","Management".');
		...
		section_content('<fieldset>
		<legend>Ad Blocks</legend>
		<p>
		'.label('Application Header').'
		Recommended size: 468*60<br />
		<textarea name="appad_header" style="width:550px;height:90px">'.h(get_gvar('appad_header')).'</textarea>
		</p>
		<p>
		'.label('Application Footer').'
		Recommended size: 468*60<br />
		<textarea name="appad_footer" style="width:550px;height:90px">'.h(get_gvar('appad_footer')).'</textarea>
		</p>
		');
			section_content('
			<p>
			'.label('Application Sidebar Footer').'
			Recommended size: 200*200<br />
			<textarea name="appad_sidebar" style="width:550px;height:90px">'.h(get_gvar('appad_sidebar')).'</textarea>
			</p>
			<p>
			'.label('Ads Bar').'
			Recommended size: 160(width)*600(height), or 120*600<br />
			<textarea name="appad_ad" style="width:550px;height:90px">'.h(get_gvar('appad_ad')).'</textarea>
			</p>
			<p>
			'.label('Search result page').'
			Recommended size: 200(width)*200(height), or 300*300<br />
			<textarea name="ad_block_search" style="width:550px;height:90px">'.h(get_gvar('ad_block_search')).'</textarea>
			</p>
			');
	...
	function ad_blockspost() {
		...
		set_gvar('appad_ad', $_POST['appad_ad']);
		set_gvar('appad_sidebar', $_POST['appad_sidebar']);
		set_gvar('appad_header', $_POST['appad_header']);
		set_gvar('appad_footer', $_POST['appad_footer']);
		set_gvar('ad_block_search', $_POST['ad_block_search']);
	...
	function customfields() {
		...
		<tr>
			<td><input type="text" size="20"  value="Do you Smoke?" /></td>
			<td>
			<select>
