...
class account {
	...
	function index($onreg = 0) {
		global $content, $db, $nav, $client, $sub_menu, $locations;
		if ($onreg) {
			sys_notice(t('Please submit the required fields'));
		}
		if ($client['id']) {
		...
		section_content('
					<form method="post" name="form1" action="'.url('account/post').'"  enctype="multipart/form-data">
					
				
					
					<fieldset>
					<legend>'.t('My details').'</legend>
					<p>
					'.label('*'.t('Full Name')).'
					...
					foreach($locations as $location) {
						if ($row['location'] == trim($location)) {
							$selected = 'selected';
						}
						else {
							$selected = '';
						}
						section_content('<option value="'.$location.'" '.$selected.' >'.$location.'</option>');
					}
					section_content('</select>
					</p>
	...
	function post() {
		...
		for($i=1;$i<=7;$i++) {
			...
			if ($ctype != 'disabled' && get_gvar($key5)) {
				if (!strlen($_POST[$col])) {
					sys_back(t('Please fill in all the required blanks'));
				}
			}
		}
		$res = sql_query("select * from `".tb()."accounts` where id='{$client['id']}'");
		...
		if (!$_POST['fullname']) {
			$newacc['fullname'] = $client['username'];
		}
		else {
		...
		if (sql_update($newacc,tb().'accounts')) {
			...
			sys_back('cannot update userinfo');
		}
	}
	...
	function privacy() {
		...
		section_content('
					...
					section_content('
					<select name="profile_permission">
					<option value="0" '.$profile_permission_0.'>'.t('Everyone can view my profile, and comment on my wall').'</option>
					<option value="1" '.$profile_permission_1.'>'.t('Everyone can view my profile, but only my friends can comment on my wall').'</option>
					<option value="2" '.$profile_permission_2.'>'.t('Only my friends can view my profile, and comment on my wall').'</option>
					</select>
					</p>
					<p>
