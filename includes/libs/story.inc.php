...
class story{
	...
	function liststories($uri) {
		GLOBAL $db,$num_per_page,$page,$ubase,$offset,$content,$client,$title,$sub_menu, $cat_id, $uhome, $current_sub_menu;
		if (eregi("^page_",$uri)) {
			$arr = explode('_',$uri);
			$page_id = $arr[1];
			if (!eregi("^[0-9]+$",$page_id)) die('bad uri');
			...
			if ($uri == 'following') {
				...
				$res2 = sql_query("select f.fid from ".tb()."followers as f left join ".tb()."accounts as u on u.id=f.fid where f.uid='{$client['id']}' order by u.lastlogin desc limit 50");
			...
			elseif ($uri == 'mine') {
				...
				$uri = 'all';
				$res = sql_query("SELECT s.*,u.username FROM `".tb()."stories` as s left join `".tb()."accounts` as u on u.id=s.uid where s.app='{$this->name}' ORDER by s.id DESC LIMIT $offset,$num_per_page ");
			}
		...
		if ($res) {
			...
			// pager
			$res = sql_query("select count(*) as total from `".tb()."stories` where  app='{$this->name}' $pwhere");
			$row = sql_fetch_array($res);
			$total = $row['total'];
			$pb       = new PageBar($total, $num_per_page, $page);
			$pb->paras = $ubase.$this->name.'/'.$uri;
			$pagebar  = $pb->whole_num_bar();
			section_content($pagebar);
		}
		else {
			c(t('No entry'));
		}
	...
	function theme_list_lines($row) {
		global $client;
		section_content('<div class="story">');
		if (!$row['thumbnail'] && $this->default_thumb) {
			$row['thumbnail'] = $this->default_thumb;
		}
		if ($row['thumbnail']) {
			section_content('<div class="thumb">'.url($this->name.'/viewstory/'.$row['id'],$this->list_thumbnail($row)).'</div>');
		}
		section_content('<div class="header">'.$this->list_title($row).'</div>');
		section_content('<div class="content">'.$this->list_summary($row).'</div>');
		section_content('<div class="tab_things">');
		$content .= $this->theme_story_footer($row);
		if (method_exists($this,'hook_list_lines')) {
			$content .= $this->hook_list_lines($row);
		}
		if ($client['id'] == $row['uid'] || allow_access(3)) {
			section_content('<div class="tab_thing">'.url($this->name.'/editstory/'.$row['id'], t('Edit')).'</div>');
			section_content('<div class="tab_thing">'.url($this->name.'/deletestory/'.$row['id'], t('Delete')).'</div>');
		}
		section_content('</div>');
		section_content('</div>');
		return $content;
	...
	function theme_list_gallery($row) {
		...
		if ($row['photos'] > 1) {
			section_content('('.$row['photos'].' pics)');
		}
...
'.$current_menu_path.'
			...
			if ($row['photos'] && $this->name == 'photos') {
				...
				while ($photo = sql_fetch_array($res)) {
					$story .= '<li><a title="'.htmlspecialchars($photo['des']).'" href="'.uhome().'/'.$photo['uri'].'" ><img src="'.uhome().'/'.$photo['thumb'].'" /></a></li>';
				}
				$story .= '</ul></div>';
			}
	...
	function writestorypost() {
		...
		if (sql_insert($story, tb().'stories')) {
			...
			if (strlen($this->act_write)) {
				...
				//$stream_id = stream_publish($this->act_write,$attachment,$app);
				$set_story['id'] = $sid;
	...
	<script language="javascript" type="text/javascript">
		...
		language : "en",
		theme_advanced_buttons1 : "code,bold,italic,underline,bullist,image,undo,redo,link,unlink,forecolor,removeformat,cleanup",
	...
	function view_content($row) {
		...
		$output .= $this->show_content($row['content']).'</div>';
		return $output;
	}
