...
function do_auth($roleids, $force_uid = 0, $msg = '') {
	...
	if (!allow_access($roleids, $force_uid)) {
		...
		if (!is_array($roleids)) {
			if (!$roleids) {
				$need_roles = '<br />No Roles was allowed';
			}
		}
		else {
			if (!$roleids[0]) {
				...
				$need_roles = t('<br />The following roles have the permission').':<ul>';
...
function url($link,$name = 'ohno',$target='') {
	GLOBAL $ubase, $uhome;
	if ($name == '') {
		$name = $link;
	}
	if (eregi("^http",$link)) {
		...
		if (eregi("^account",$link) || eregi("^admin",$link) || eregi("^login",$link) || $link == 'logout') {
...
function save_file($file, $allowed_ext = array('jpg','png','gif')) {
	...
	$ext = strtolower($ext);
	if (eregi("\.",$ext)) $ext = substr($ext,-2,2);
...
function limit_posting($checking = 0,$ajax = 0) {
	global $client;
	if ($client['disabled'] == 2) {
		if ($ajax) {
			die('<div style="color:red">'.t('Your account is currently pending approval by administrators').'</div>');
		}
		else {
			sys_back(t('Your account is currently pending approval by administrators'));
		}
	}
	elseif($client['disabled'] == 1) {
		if ($pending_post_limit = get_gvar('pending_post_limit')) {
			$res = sql_query("select count(*) as num from ".tb()."streams where uid='{$client['id']}'");
			$row = sql_fetch_array($res);
			if (($row['num']-1) < $pending_post_limit) {
				return true;
			}
		}
		if ($ajax) {
	...
	if (!$client['id'])
		return false;
	$limit = get_gvar('limit_posting_num');
	if (!$limit)
		return true;
	
	$timeline = time() - 100*3600;
	if ($client['created'] < $timeline) {
		return true;
	}
	
	$togglekey = 'memberposttoggle'.$client['id'];
	$tkey = 'memberpost'.$client['id'];
	if ($toggle = get_tmp($togglekey)) {
		if (time() > ($toggle+12*3600)) {
			return true;
		}
		else {
			if ($ajax) {
				die(t('You have exceeded you posting limit, please wait for 12 hours. By that time, your posting will be Unlimited.').'<br />');
			}
			else {
				sys_back(t('You have exceeded you posting limit, please wait for 12 hours. By that time, your posting will be Unlimited.'));
			}
		}
	}
	$current_posts = get_tmp($tkey);
	if ($current_posts >= $limit) {
		set_tmp($togglekey, time());
		set_tmp($tkey,0);
		sys_back(t('You have exceed you posting limit, please wait for 12 hours. By that time, your posting will be Unlimited.'));
	}
	if (!$current_posts) $current_posts = 0;
...
function stream_form($preset='',$page = array()) {
	...
	elseif ($preset == 'video') {
		...
		if ($page['uid'] == $client['id']) {
			$default_msg = t("What's happening...");
		}
		else {
	...
	if (is_array($quick_apps) and count($quick_apps) > 0) {
		...
		foreach ($quick_shares as $quick_share) {
			...
			$("#stream_att_'.$app.'_radio").click(function() {
					...
					$("#apps_box").load("'.uhome().'/'.$app.'/ajax_form/'.$page_type.'/'.$page['id'].'", function() {
	...
	$output =  
		...
		$(document).ready( function(){
				...
				function showRequest() {
					...
					$("span#spanstatus").html("<img src=\"'.uhome().'/files/loading.gif\" /> Submitting");
					$("#charsRemaining").html("");
		...
		<div id="post_form">
			<div id="stream_att">
				<a href="javascript:void();" class="att_button'.$status_att_button_on.'" id="stream_att_status_radio"><img src="'.uhome().'/files/appicons/status.png" align="absmiddle" /> Status</a>
				'.$a_button.'
			</div>
			...
			<div id="stream_box">
				...
				<textarea name="message" rows="2" id="form_message" maxlength="'.$miniblog_maximum.'" class="fpost"  style="width:95%" >'.$default_msg.'</textarea>
				<div id="apps_box"></div>
				...
				<input type="submit" value=" '.t('Share').' " class="fbutton" id="form_submit" disabled />
				</td>
				</tr>
				</table>
				</form>
...
function comment_form($stream_id, $flag = '', $likeflag = '') {
	...
	if ($stream['aid'] && ($client['id'] == $stream['uid'])) {
		$likeit = '<span></span>';
		$no_like = 1;
	}
	else {
