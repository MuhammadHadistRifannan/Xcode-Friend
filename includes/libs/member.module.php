...
class member{
	function login($need_login = 0, $iid = 0) {
		...
		set_title('Log In');
		if ($client['id']) redirect('dashboard');
		if ($need_login)
...
<form method="post" name="form1" id="form1" action="'.url('member/loginpost').'" >
	...
	function chpassdo($code = '') {
		if (!eregi("^[0-9a-z]+",$code)) die('not a valid code');
...
<tr class="row1">
<td>*'.t('Email Address').' '.$use_i_email.'</td>
<td>
...
<td>'.t('About me').'</td><td>
			...
			<div style="width:700px;height:200px;overflow:scroll;border:white 2px solid;padding:5px;">
	...
	function need_verify() {
		...
		elseif (get_gvar('acc_verify') == 2) {
			c(t('Your account is currently pending approval by administrators'));
		}
		else {
