...
$dbtype = 'MySQL';
$homeurl = "http://".$_SERVER['HTTP_HOST'].ereg_replace("/install.php","",$_SERVER['REQUEST_URI']);
if (eregi('localhost',$homeurl) || eregi("^192.168",$homeurl) || eregi("^10.2.",$homeurl) || eregi("^127.0.0",$homeurl)) {
	$testing = 1;
}
// header ----------------------------------------------------------------------------------------------------------------------
?>
...
<tr>
<td align="right" class="row2" valign="top">Administrator email</td><td class="row2">
	<input type="text" name="email" size="40"  /></td>
</tr>

<tr>
<td align="right" class="row2" valign="top">Administrator password</td><td class="row2">
...
elseif ($_POST['step'] == 2) {
	if (eregi("\/$",$_POST['uhome'])) $_POST['uhome'] = substr($_POST['uhome'],0,strlen($_POST['uhome'])-1);
...
<br />
You can now login to your Network with the admin account:<br />
