    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div class="lg:col-span-6 space-y-6">
            ...
            @if($tab === 'dinding')
                ...
                <div id="feed-stream-container" class="space-y-6" data-wall-uid="{{ $profileUser->id }}">
                    @forelse ($streams as $stream)
                        <x-single-stream :stream="$stream" />
                    @empty
                        <div id="feed-empty-state" class="text-center text-sm text-neutral-500 py-10 bg-white rounded-xl shadow-sm border border-neutral-200">
                            Tidak ada postingan di dinding.
                        </div>
                    @endforelse
                </div>

                <!-- Sentinel Infinite Scroll & Skeleton Shimmer Loading -->
                <div id="feed-infinite-sentinel" class="py-4 text-center"
                     data-has-more="{{ $streams->hasMorePages() ? '1' : '0' }}"
                     data-next-page="{{ $streams->nextPageUrl() }}"
                     style="display: {{ $streams->hasMorePages() ? 'block' : 'none' }};">
                ...
                <div id="feed-no-more" class="text-center py-6 text-xs text-neutral-400 font-medium {{ $streams->hasMorePages() ? 'hidden' : '' }}">
