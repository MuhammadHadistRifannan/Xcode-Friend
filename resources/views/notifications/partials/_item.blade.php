<form action="{{ route('notifications.markRead', $notification->id) }}" method="POST" class="block">
    @csrf
    <div class="flex items-center gap-4 px-6 py-3 {{ !$loop->last ? 'border-b border-gray-100' : '' }} {{ $notification->hasread ? '' : 'bg-[#fafafa]' }} hover:bg-gray-50 transition cursor-pointer" onclick="this.closest('form').submit()">
        <!-- Unread Indicator -->
        ...
        <div class="flex-grow min-w-0">
            ...
            <span class="text-xs text-gray-500 ml-2">{!! $notification->message !!}</span>
        </div>
