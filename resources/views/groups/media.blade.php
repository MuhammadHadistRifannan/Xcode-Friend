    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            <div class="flex items-center gap-2 text-sm text-gray-500 mb-2">
                <a href="{{ route('groups.show', $group->id) }}" class="hover:text-red-700 transition-colors">{{ $group->name }}</a>
                <i data-lucide="chevron-right" class="w-4 h-4"></i>
                <span class="text-gray-900 font-semibold">{{ $title }}</span>
            </div>
            <h1 class="text-2xl font-bold text-gray-900">{{ $title }}</h1>
        </div>
        <a href="{{ route('groups.show', $group->id) }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold py-2 px-4 rounded-lg flex items-center gap-2 transition-colors text-sm">
